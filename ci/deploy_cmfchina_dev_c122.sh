...
#C122
run rsync -av --delete --exclude=.git  dist_cmfchina/ ci@100.64.0.11:/data/trident_cmfchina_dev/code_src/trident/user_proxy/static

if [[ -f "/data/ci/fitout/autodoc/send_mm_msg.sh" ]]; then
  bash /data/ci/fitout/autodoc/send_mm_msg.sh http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa trident "[Trident(应用门户)-招商基金-前端-测试环境](http://100.64.0.11:22102)已更新至版本:\`${GO_REVISION_FRONT_TRIDENT:0:8}(${GO_MATERIAL_BRANCH_FRONT_TRIDENT})\`"
fi
