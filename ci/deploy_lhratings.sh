...
#C13
run rsync -av --delete --exclude=.git ./dist/ ci@100.64.0.13:/data/autodoc_lhratings_test/trident_code/trident/user_proxy/static

if [[ -f "/data/ci/fitout/autodoc/send_mm_msg.sh" ]]; then
  bash /data/ci/fitout/autodoc/send_mm_msg.sh http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa trident "[Trident(应用门户)-联合资信-前端-测试环境](http://100.64.0.13:25700/)已更新至版本:\`${GO_REVISION_FRONT_TRIDENT:0:8}(${GO_MATERIAL_BRANCH_FRONT_TRIDENT})\`"
fi
